...
ServerEvents.recipes((event) => {
  ...
  itemsToRemove.forEach((itemId) => {
    // Remove by output (recipe result) - Safe now that conflicting mod is gone
    event.remove({ output: itemId });
  });

  console.info(
    `[Arcadia V2] Attempting to remove recipes for ${itemsToRemove.length} items.`,
  );

  console.info(`[Arcadia V2] ${itemsToRemove.length} item recipes removed.`);
});
