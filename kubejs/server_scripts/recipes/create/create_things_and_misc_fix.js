// Rebuilds the create_things_and_misc recipes that 1.21 NeoForge refuses to load.
// Issue: numeric keys in the shaped pattern, which the 1.21 codec rejects.
// Author: vyrriox
//
// Scope check against create_things_and_misc 4.1.1: the mod has since fixed the
// legacy 1.20 result format and the forge: tags on its own side, so the 26
// rebuilds that covered those are gone and the mod's own recipes apply again.
// The 13 below are the ones still shipped with a numeric pattern key.
//
// Note: the jar also carries a data/create_things_and_misc/recipes/ folder, the
// 1.20 path. 1.21 only reads recipe/, so nothing in there loads and none of it
// needs patching here.
...
ServerEvents.recipes(event => {
    ...
    console.info('[Arcadia V2] create_things_and_misc fix: 13 recipes rebuilt (numeric pattern keys).');
});
