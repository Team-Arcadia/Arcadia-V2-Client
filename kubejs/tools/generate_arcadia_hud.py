...
TRANSPARENT = (0, 0, 0, 0)
BLACK_IRON = (14, 16, 17, 245)
DARK_IRON = (28, 30, 29, 245)
STEEL = (91, 94, 89, 255)
...
def build_hotbar() -> Image.Image:
    ...
    draw.line((2, 1, 179, 1), fill=STEEL)
    draw.line((2, 2, 179, 2), fill=DARK_COPPER)
    draw.line((2, 19, 179, 19), fill=(45, 28, 20, 255))
    ...
    for slot in range(9):
        ...
        draw.line((left + 2, 3, right - 1, 3), fill=(53, 50, 42, 255))
        draw.line((left + 2, 18, right - 1, 18), fill=(12, 14, 14, 255))
        if slot < 8:
            draw.line((right, 3, right, 18), fill=DARK_COPPER)
            draw.line((right + 1, 3, right + 1, 18), fill=(113, 86, 47, 255))
...
def build_selection() -> Image.Image:
    ...
    draw.line((3, 0, 20, 0), fill=BRASS)
    draw.line((3, 22, 20, 22), fill=DARK_COPPER)
    draw.line((0, 3, 0, 19), fill=BRASS)
    draw.line((23, 3, 23, 19), fill=DARK_COPPER)
    draw.line((1, 2, 2, 1), fill=PALE_BRASS)
    draw.line((21, 1, 22, 2), fill=COPPER)
    draw.line((1, 20, 2, 21), fill=COPPER)
    draw.line((21, 21, 22, 20), fill=DARK_COPPER)

    draw.rectangle((2, 2, 21, 20), outline=COPPER)
    draw.rectangle((3, 3, 20, 19), outline=PALE_BRASS)
    draw.point((3, 3), fill=ARCANE_CYAN)
    draw.point((20, 3), fill=ARCANE_CYAN)
    draw.point((3, 19), fill=(25, 114, 119, 255))
    draw.point((20, 19), fill=(25, 114, 119, 255))
    return image
