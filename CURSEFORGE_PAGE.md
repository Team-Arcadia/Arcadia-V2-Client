<!--
  ...
  Paste the HTML below (everything from the first <h1> to the last </p>) into
  CurseForge's "Description" editor (HTML mode). Keep in sync at each release.

  Design notes (v3):
    - GIFs reduced to 9 unique (each shown once). Removed world / header /
      branding GIFs. Kept only mechanic showcase GIFs.
    - Badge rows now have explicit vertical spacing (no more cramped feel).
    - WIP notice, Quality of Life, Multiplayer, and Technical Excellence
      sections rebuilt as bilingual / dual-pillar callout cards (yellow,
      blue, orange borders) — visual hierarchy now matches the page tone.
    - System Requirements table now has zebra-banded rows + bordered cells.
    - "Cross-mod bridges" feature row added to fill the previously sparse
      gap between progression and combat sections.
    - FR and EN sections strictly parallel in structure, assets, and length.
    - All wiki / discord / website CTAs surface in header + footer.
-->
