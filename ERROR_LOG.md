...
**Prevention:** `type: "loot"` and `type: "random"` both fail closed and silently when `table_id` is absent or points at a deleted table — no warning at load, no warning at claim. After any reward-table edit, sweep the chapters for reward entries whose `type: "loot"` or `type: "random"` line is not immediately preceded by a `table_id` line (FTB Quests writes SNBT keys alphabetically, so `table_id` always sits directly above `type`). A full sweep on 2026-08-11 found this entry plus one more, quest `4D4AB60B3B1CD437` in `twilight_forest.snbt`, whose `type: "random"` reward has no table either. That chapter uses no reward tables at all, so the correct table is a balance decision and it was left untouched pending a call.
